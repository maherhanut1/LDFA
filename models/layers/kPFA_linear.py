...
class LinearGrad(autograd.Function):
    ...
    def forward(context, input, weight, weight_backward, R, bias=None, bias_backward=None):
        context.save_for_backward(input, weight, weight_backward, R, bias, bias_backward)
    ...
    def backward(context, grad_output):
        input, weight, weight_backward, R, bias, bias_backward = context.saved_tensors
        grad_input = grad_weight = grad_weight_backward = grad_bias = grad_bias_backward = R_grad = None
        # Gradient input
        if context.needs_input_grad[0]:
            # Use the weight_backward matrix to compute the gradient
            grad_input_B = grad_output.mm(weight_backward)
            grad_input = grad_input_B.mm(R)
            R_grad = grad_input_B.t().mm(input)
        # Gradient weights
        ...
        return grad_input, grad_weight, grad_weight_backward, R_grad, grad_bias, grad_bias_backward


class Linear(nn.Linear):
    def __init__(self, in_features: int, out_features: int, rank, bias: bool = True, layer_config: dict = None) -> None:
        ...
        self.weight_backward = nn.Parameter(torch.Tensor(self.weight.size()[0], self.rank), requires_grad=False)
        self.R = nn.Parameter(torch.Tensor(rank, self.weight.size()[1]), requires_grad=True)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return LinearGrad.apply(x, self.weight, self.weight_backward, self.R, self.bias, self.bias_backward)
